...
```
weft-app-windows (tray)                     weft-app-windows --dashboard
─────────────────────                       ────────────────────────────
...
```powershell
weft-app-windows.exe --config app.json --wg-config wireguard.json
```
...
```
   ┌─────────────────┐    ┌──────────────────────┐   ┌────────────────────┐
   │ %USERPROFILE%\  │    │ Windows Credential   │   │ weft-app-windows   │
...
#    history or env vars.
weft-app-windows.exe --store-ssh-passphrase %USERPROFILE%\.ssh\id_ed25519
...
`weft-app-windows --print-pubkey` and pasting the output into the
server's allowlist.
